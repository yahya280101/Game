...
public class Game {
	...
	public static double evaluate(ArrayList <Unit> maximizer,ArrayList <Unit> minimizer) {
	    double maxCurrentSoldierCount=0;
	    for(int i=0;i<maximizer.size();i++) {
	    	maxCurrentSoldierCount+=maximizer.get(i).getCurrentSoldierCount();
	    }
	    double minCurrentSoldierCount=0;
	    for(int i=0;i<minimizer.size();i++) {
	    	minCurrentSoldierCount+=minimizer.get(i).getCurrentSoldierCount();
	    }
	    return maxCurrentSoldierCount-minCurrentSoldierCount;
	}
	public static double miniMax(ArrayList <Unit> maximizer,ArrayList <Unit> minimizer,int depth,boolean maximizingPlayer){
		if(depth==0 || maximizer.size()==0 || minimizer.size()==0) {
			return evaluate(maximizer,minimizer); 
		}
		
		if(maximizingPlayer) {
			double maxEval= Double.NEGATIVE_INFINITY;
			for(Unit u1:maximizer) {
				for(Unit u2:minimizer) {
					try {
						u1.attack(u2);
					...
					double eval=miniMax(maximizer,minimizer,depth-1,false);
			...
			for(Unit u1:minimizer) {
				for(Unit u2:maximizer) {
					try {
						u1.attack(u2);
					} catch (FriendlyFireException e) {
						// TODO Auto-generated catch block
					...
					double eval=miniMax(maximizer,minimizer,depth-1,true);
    ...
    public static void main(String[] args) {
		...
		Unit T1=new Archer(1, 60, 0.4, 0.5, 0.6);
		Unit T2=new Cavalry(3, 60, 0.7, 0.8, 0.9);
		Unit T3=new Cavalry(1, 60, 0.5, 0.6, 0.7);
		Unit T4=new Cavalry(1, 60, 0.5, 0.6, 0.7);

		T1.setParentArmy(a);
		T2.setParentArmy(a);
		T3.setParentArmy(a);
		T4.setParentArmy(a);
		Unit R1=new Archer(2, 60, 0.4, 0.5, 0.6);
		Unit R2=new Archer(1, 60, 0.4, 0.5, 0.6);
		Unit R3=new Infantry(1, 60, 0.4, 0.5, 0.6);
		Unit R4=new Infantry(1, 60, 0.4, 0.5, 0.6);
      ...
      System.out.println(miniMax(a.getUnits(),b.getUnits(),1,true));
      System.out.println(evaluate(a.getUnits(),new ArrayList<Unit>()));
      System.out.println(evaluate(b.getUnits(),new ArrayList<Unit>()));
      System.out.println(evaluate(a.getUnits(),b.getUnits()));
